...
idx_file="indices_list.npy"

sub_sample_rate=0.875
max_ind=10000000
data="--data-generation=$data_type --data-set=$dataset --raw-data-file=$raw_data_file --processed-data-file=$processed_data_file --data-sub-sample-rate=$sub_sample_rate --max-ind-range=$max_ind --memory-map --save-indices --indices-file=$idx_file"
...
echo "Running DLRM (Saving Indices)..."
